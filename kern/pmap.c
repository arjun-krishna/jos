...
boot_alloc(uint32_t n)
{
	...
	if( n != 0) {
		char* next = nextfree;
		nextfree = ROUNDUP((char* )(nextfree+n),PGSIZE); // allocate pages which can handle n bytes
		return next;
	} else {
		return nextfree;
	}
	...
	if(!(pgdir[dirIndex] & PTE_P)) {
		if(create) {
			...
			if (!pg)
				return NULL;
			pg->pp_ref++;
			pgdir[dirIndex] = page2pa(pg) | PTE_P | PTE_W | PTE_U;
		} else return NULL;
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	// 	if (!p) {
	// 		panic("in boot_map_region : out of memory");
	// 	}
	// 	*p = pa | perm | PTE_P;
	// 	va+=PGSIZE;
	// 	pa+=PGSIZE;
